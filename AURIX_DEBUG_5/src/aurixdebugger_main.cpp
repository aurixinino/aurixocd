...
{
	bool bFoud=false;
	uint16_t uiOpCode = uiInstruction&0xFF;
	int i=0;

	for (int i=0; ( (i<NUMOPCODES) && !bFoud); i++)
	{
		if (uiOpCode == tc16[i].uiOpCode)
		{
			bFoud = true;
...
void Debug_Trace(mcd_core_st *core, uint32_t i_core, mcd_return_et ret)
{
	...
	txDemo.addr.address = 0xF881FE08 + (i_core * 0x20000);
	uiPC = 0xEEEEEEEE;  
	ret = mcd->read32(core, &txDemo.addr, &uiPC);
	mcdd_handle_err(stdout, &core, ret);

	// Acquire the Instruction (as pointed by the PC)
	txDemo.addr.address = uiPC;
	...
	mcdd_handle_err(stdout, &core, ret);
	printf("PC@0x%8.8X INSTRUCTION 0x%8.8X ", uiPC, uiInstruction);
