...
int main(int argc, char** argv) 
{
  ...
  while (true) {
	...
	if ( strncmp(cmd, "r", 1) == 0) {				
		...
		txDemo.addr.address = 0xF881FF80 + (i_core * 0x20000);
		for (int i=0; i<16; i++) {
			if (i%4 == 0) printf("\n");
			value = 0xEEEEEEEE;   
			ret = mcd->read32(core, &txDemo.addr, &value);
			mcdd_handle_err(stdout, &core, ret);
			printf("A%2.2d=0x%8.8X ", i, value);
			txDemo.addr.address += 4;
		}

		// Loop on Data registers Dx
		txDemo.addr.address = 0xF881FF00 + (i_core * 0x20000);
		for (int i=0; i<16; i++) {
			if (i%4 == 0) printf("\n");
			value = 0xEEEEEEEE;   
			ret = mcd->read32(core, &txDemo.addr, &value);
			mcdd_handle_err(stdout, &core, ret);
			printf("D%2.2d=0x%8.8X ", i, value);
			txDemo.addr.address += 4;
		}
		printf("\n");
		continue;
...
{
	printf("\n\n");
	printf(    "Control core:         run, stop, step <steps>\n");
